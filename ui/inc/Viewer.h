...
/// @brief viewer, performs display actions using registered objects via
/// registerViewerFunction
class Viewer {
public:
  enum class ViewType { Flat, PointCloud };
  struct Config {
    unsigned int x;
    unsigned int y;
    unsigned int w;
  ...
  /// starts displaying using Config passed as a ctor, shall be used as a main
  /// thread loop
  void start() const;
  ...
  void registerViewerFunction(void (*func)(const Object *), const Object *obj) {
    viewerFuncitons.push_back(ViewerFunction{
        .function = reinterpret_cast<void (*)(const void *)>(func),
        .worker = obj});
  }
