...
MapGlUByte3 valueRangeToRGBByte(const unsigned int minv,
                                const unsigned int maxv,
                                const unsigned int value) noexcept {
  const auto ratio =
      static_cast<float>(value - minv) / static_cast<float>(maxv - minv);
  ...
  case 0:
    return {255u, y, 0u};
  case 1:
    return {static_cast<GLubyte>(255u - y), 255u, 0u};
  case 2:
    return {0u, 255, y};
  case 3:
    return {0u, static_cast<GLubyte>(255u - y), 255u};
  case 4:
    return {0u, 0u, 255u};
  }
  return {0, 0, 0};
...
tVal mapValue(std::pair<tVal, tVal> a, std::pair<tVal, tVal> b,
              tVal inVal) noexcept {
...
void lidar3D_Display(const dev::CygLidarD1 *lidar) {
  ...
  constexpr std::pair<GLfloat, GLfloat> glRangeX = {
      0.f, static_cast<GLfloat>(frameResolution.first)};
  constexpr std::pair<GLfloat, GLfloat> glRangeY = {
      0.f, static_cast<GLfloat>(frameResolution.second)};
  constexpr std::pair<GLfloat, GLfloat> glRangeZ{
      static_cast<GLfloat>(minimumDistanceMm),
      static_cast<GLfloat>(maximumDistanceMm)};

  if (pointCloud.size() != frameResolution.first * frameResolution.second) {
    throw std::runtime_error(
        "mismatched frameResolution and pointCloud.size(), they should match!");
  }
  ...
  for (auto y = 0u; y < frameResolution.second; ++y) {
    for (auto x = 0u; x < frameResolution.first; ++x) {
      ...
      if (pointCloud[frameId] == 4082u) {
        ...
        positionMap = {
            mapValue<GLfloat>(glRangeX, glFullScreenRangeX,
                              static_cast<GLfloat>(x)),
            mapValue<GLfloat>(glRangeY, glFullScreenRangeY,
                              static_cast<GLfloat>(y)),
            mapValue<GLfloat>(glRangeZ, glFullScreenRangeZ,
                              static_cast<GLfloat>(pointCloud[frameId]))};
      ...
      const auto rgbValues = valueRangeToRGBByte(
          minimumDistanceMm, maximumDistanceMm, pointCloud[frameId]);
      glColor3ubv(&rgbValues[0]);
      positionMap = {
          mapValue<GLfloat>(glRangeX, glFullScreenRangeX,
                            static_cast<GLfloat>(x)),
