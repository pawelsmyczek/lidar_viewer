...
{
    return static_cast<ByteType>(::roundf(static_cast<float>(value) * static_cast<float>(scalar)));
}
...
void
rotateAroundX(lidar_viewer::geometry::types::Point3D<CoordType> &positionMapCpy, CoordType rotationCenter, const float rotationValueX)
{
    const auto sinX = sinf(rotationValueX);
    const auto cosX = cosf(rotationValueX);

    positionMapCpy[1] = rotationCenter; // move to the center

    positionMapCpy[1] = positionMapCpy[1] * cosX - positionMapCpy[2] * sinX;
    positionMapCpy[2] = positionMapCpy[1] * sinX +  positionMapCpy[2] * cosX;
}

template <typename CoordType>
void
rotateAroundY(lidar_viewer::geometry::types::Point3D<CoordType> &positionMapCpy, CoordType rotationCenter, const float rotationValueY)
{
    // rotate around Y
    const auto sinY = sinf(rotationValueY);
    const auto cosY = cosf(rotationValueY);

    positionMapCpy[0] = rotationCenter; // move to the center

    positionMapCpy[0] = positionMapCpy[0] * cosY + positionMapCpy[2] * sinY;
    positionMapCpy[2] = -positionMapCpy[0] * sinY +  positionMapCpy[2] * cosY;
}
